...
- **续行运算符对齐**:括号内同一缩进级别的连接符 (`and`/`or`/`not` 及邻近/频率算符) **右对齐在同一列**,使其后的关键词、字段名、括号块都落在同一内容列——`or` 比 `and` 窄一列,故其后补 2 个空格、`and`/`not` 补 1 个;长度 ≥ 4 的连接符右**不留空格**、紧跟后一个片断的括号;更宽的连接符(`(sen)`、`(57w)`、`(99n)`)则整体**左移**超出 4 列的部分,使其后的括号仍与前面的括号同列
...
- **邻近表达式作为链中的一项时整体加括号**(便于看出关系的作用范围),放不下时外层括号独占一行、内部再缩进 8 格;块跨多行时它后面的链上内容(如 `or 关键词`)会另起一行,例如:
