...
- **续行运算符对齐**:括号内的续行连接符统一落在缩进列(括号列 +4),其后补足空格使关键词与首行内容列(括号列 +8)对齐——`or` 后补 2 个空格,`and`/`not`/`(s)` 后补 1 个空格;长度 ≥ 4 的连接符(`(3w)`、`(sen)`、`(99n)` 等)右侧**不留空格**,直接紧跟后一个片断的括号,使前后两个括号尽量对齐(比 +4 更宽的连接符无法完全对齐)
