...
- **邻近运算符 (s)/(p)/(Nw)/(Nn) 两侧的值片断、频率运算符 (Nf) 前的值片断自动加括号**,方便并列近义词:`温差 (s) 蒸发器` → `(温差) (s) (蒸发器)`,`data (2w) line` → `(data) (2w) (line)`,`"机器人" (3f)` → `("机器人") (3f)`
...
- **续行运算符对齐**:括号内的续行连接符统一落在缩进列(括号列 +4),其后补足空格使关键词与首行内容列(括号列 +8)对齐——`or` 后补 2 个空格、`and`/`not`/`(s)` 后补 1 个空格;比该偏移更宽的连接符(如 `(99n)`)只保留 1 个分隔空格
- **邻近运算符 (s)/(p)/(Nw)/(Nn) 两侧的值片断保持可见**:能放进一行时写作 `(片断) (s) (片断)`;放不下时运算符另起一行并落在缩进列,两侧括号同列,各自括号内的关键词独立换行,例如:
