...
| `(sen)` | `词1 (sen) 词2` | 同 `(s)`，两词出现在同一句子中（`sen` = sentence） |
...
频率算符限定某个词在文档中出现的最少次数。`n` 为 **1~100** 的整数,表示该关键词在该字段中出现频率 ≥ n。频率算符的**优先级高于逻辑运算符**。

| 算符 | 格式 | 说明 |
|------|------|------|
| `(nf)` | `词 (nf)` | 该词至少出现 n 次(n = 1~100) |

**示例**：
```
tiab=("机器人"(3f))                        -- "机器人"在标题/摘要中至少出现3次
tiabc=("机器人"(3f) AND "遥感"(3f))         -- 支持与逻辑算符连用
des=((机器人 OR 红外)(3f))                  -- 支持与逻辑算符嵌套使用
tiab=("机器人"(100f))                       -- n 最大为 100
```

**官方限制(不要这样写)**：

1. 不支持与位置算符 `(w)`/`(n)`、同句算符 `(s)`、同段算符 `(p)` 连用或嵌套,如 `TI=((clectronic(3n)vehicle)(3f))`
2. 不支持对包含截词符的关键词使用,如 `TIAB=(syste$m*(3f))`
3. 频率算符不支持嵌套使用,如 `TI=((机器人)(3f) AND 红外)(3f)`
4. 系统不支持频率算符与表达式嵌套使用,如 `(TI=机器人)(3f)`

> 本插件对第 4 类写法会把它移入字段值内部(`ti = ((机器人) (3f))`),其余写法则按语法错误处理。

### 4b. 可选运算符 OPT (Optional)

`OPT` 用于限定"可选检索要素"并做优先级排序:包含的检索要素越多、且多个位置都包含检索要素的专利排得越靠前。

| 写法 | 说明 |
|------|------|
| `(条件) OPT (可选要素)` | `OPT` 前的所有布尔检索要素要用括号包起来;`OPT` 后的可选检索要素外层也要用括号包起来 |
| `Ti=(毛衣) OPT (AB=(编织 or 针织 or knit*))` | 同一位置内的多个可选要素之间必须用 `or` 连接 |
| `(Ti=(毛衣) and PD=[20100101 to 20250315]) OPT (AB=编织 OR AP=某某公司)` | 不同位置(字段)之间也必须用 `or` 连接 |

**示例**：
```
(Ti=(毛衣) and PD=[20100101 to 20250315]) OPT (AB=编织)

Ti=(毛衣) OPT (AB=(编织 or 针织 or 交织 or 混纺 or knit* or "sweater production"))
```

**官方限制**：

1. `OPT` 不支持与 `R`(语义检索)连用
2. `OPT` 限定的可选检索要素之间不能有 `or` 以外的关系(不能 `and`/`not`)
3. `OPT` 算符内不支持关系嵌套(如 `(AB=(编织 OR (针织 (5w) (装置 OR 烘干))))`)
